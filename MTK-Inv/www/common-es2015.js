...
let LookupService = class LookupService {
    ...
    create(Category) {
        return this.lookupCollection.add(Category);
        //return this.db.list('/Category').push(Category);
    }
    getalllookup() {
        this.lookups = this.lookupCollection.snapshotChanges().pipe(Object(rxjs_internal_operators_map__WEBPACK_IMPORTED_MODULE_3__["map"])(actions => {
    ...
    getlookupBytype(id) {
        const lookupobject = this.db.collection('lookup', ref => ref.where('type', '==', id)).snapshotChanges();
        this.lookupList = lookupobject.pipe(Object(rxjs_internal_operators_map__WEBPACK_IMPORTED_MODULE_3__["map"])(changes => changes.map(a => {
            const data = a.payload.doc.data();
            const id = a.payload.doc.id;
            return Object.assign({ id }, data);
        })));
